...
3.  Do not display (ignore) the outliers in the plot. **Note: Use a search engine or an AI tool to find the solution and add the link to the solution you have found.**
    ...
    \# Create a box plot without outliers boxplot(x, outline = FALSE, outlier.colour = "NA")

4.  Run the code contained in the code chunk and fix any errors.

5.  What are the data types of the three variables used for aesthetic mappings? Continent: distinct group; Life Exp: discrete;
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:
    ...
    **`geom_col`** is a geometric object in the **`ggplot2`** package in R that is used to create vertical bar charts. It is similar to **`geom_bar`**, but **`geom_col`** uses the **`y`** value of the data to determine the height of the bars, while **`geom_bar`** uses the count of the data
...
The **`breaks`** argument specifies the number of bins to use in the histogram and the sequence of values that define the edges of the bins. The sequence of values for the **`breaks`** argument should be generated using a positive increment.

```{r}
ggplot(data = gapminder_2007,
       mapping = aes(x = lifeExp, 
                     fill = continent)) +
  geom_histogram(col = "grey30", breaks = seq(10, 90, 5.5))
...
ggplot(data = gapminder_2007,
       mapping = aes(x = lifeExp, 
                     fill = continent)) +
  geom_histogram(col = "grey30", breaks = seq(10, 40, 3.5))
...
ggplot(data = gapminder_2007, 
       mapping = aes(x =lifeExp,
                     fill = continent)) +
  geom_histogram(col = "grey30", breaks = seq(20, 60, 4))
...
ggplot(data = gapminder_2007, 
       mapping = aes(x =lifeExp,
                     fill = continent)) +
  geom_histogram(col = "grey30", breaks = seq(20, 60, 2))
...
ggplot(data = gapminder_2007, 
       mapping = aes(x =lifeExp,
                     fill = continent)) +
  geom_histogram(col = "grey30", breaks = seq(20, 90, 2))
```

The numbers inside the de seq mean:

-   **`from`** is the starting value of the sequence. First number

-   **`to`** is the ending value of the sequence. Second number

-   **`by`** is the increment between the numbers in the sequence. Third number
...
6.  Run the code contained in the code chunk and fix any errors.
    ...
    ```{r}
    ggplot(gapminder_2007, mapping = aes(x= gdpPercap, y = lifeExp, size = pop, color = continent)) +
...
```{r}
# git_config (user.name = "Arancha", user.email = "arantxa.kine@gmail.com")

```

```{r}
library(usethis)
use_git_config(user.name = "Arancha", user.email = "arantxa.kine@gmail.com")
```
